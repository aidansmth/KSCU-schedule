...
async function fetchShow() {
    ...
    function placeData(data) { // if show is upcoming 
        if (new Date(data['start_time']) > Date.now()) {
            ...
            document.getElementById('DJ').innerHTML = data['DJ_name'] + " at " + formatAMPM(new Date(data['start_time']))
            ...
            document.getElementById("title").innerHTML = data['title'];
            document.getElementById("DJ").innerHTML = "with " + data['DJ_name'] + " till " + formatAMPM(new Date(data['end_time']));
        }
    }
